    ...
    (assert "evaluates <alternate> when <test> is NOT a true value"
            (= 10
               (if (= 6 (+ 2 3))
                      'failed
                      10)))
    (assert "<consequent> and <alternate> are never both evaluated")
    (assert "result is unspecified when <test> is #f and there is no <alternate>")
  )
  (context "(and <test1> ...)"
    (assert "returns #t if both tests evaluate to #t"
            (= #t (and (= 2 2) (= 5 5))))
    (assert "returns #t if there are no <test>s")
    (assert "returns the last <test> if no <test>s are #f")
  )
  (context "(not <obj>)"
